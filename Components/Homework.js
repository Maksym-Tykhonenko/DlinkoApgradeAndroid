...
export default function Homework({navigation, route}) {
  ...
  useEffect(() => {
    ...
    if (updatedHomework?.id) {
      setAllTasks(prev =>
        prev.map(t =>
          t.id === updatedHomework.id
            ? {...updatedHomework, completed: t.completed ?? false}
            : t,
        ),
      );
          ...
          renderItem={({item}) => {
            ...
            return (
              <View style={[styles.card, {backgroundColor: item.color}]}>
                <Image
                  source={item.photo || DEFAULT_PHOTO}
                  style={styles.homeworkImg}
                />
                <View style={styles.cardContent}>
                  <Text style={[styles.subject, done && styles.subjectDone]}>
                    {item.subject}
                  </Text>
                  <Text style={[styles.reason, done && styles.textDone]}>
                    {item.reason}
                  </Text>
                  {item.tasks?.length > 1 && (
            ...
            <SafeAreaView style={{flex: 1}}>
              <ScrollView
                ...
                <View style={styles.detailHeader}>
                  ...
                  <TouchableOpacity
                    onPress={() =>
                      setLocalDone(arr => arr.map(() => !arr.every(Boolean)))
                    }>
                    <View
                      style={[
                        styles.circle,
                        localDone.every(Boolean) && styles.circleDone,
                      ]}
                    />
                  </TouchableOpacity>
                  ...
                  <TouchableOpacity
                    onPress={() => {
                      ...
                      navigation
                        .getParent()
                        .navigate('AddHomeWork', {existingHomework: current});
                    }}>
                ...
                {/* PHOTO */}
                <Image
                  source={current.photo || DEFAULT_PHOTO}
                ...
                {current.tasks.map((t, i) => (
                  <TouchableOpacity
                    ...
                    <View
                      style={[
                        styles.circle,
                        localDone[i] && styles.circleDone,
                        {marginRight: 12},
                      ]}
                    />
                    <Text
                      style={[
                        styles.taskTextDetail,
                        localDone[i] && styles.textDone,
                      ]}>
                      {t}
                    </Text>
                  </TouchableOpacity>
